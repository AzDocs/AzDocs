...
module networkInterfaces 'networkInterfaces.bicep' = [
  for i in range(0, length(networkInterfaceInformation)): {
    ...
    params: {
      ...
      enableAcceleratedNetworking: contains(networkInterfaceInformation[i], 'enableAcceleratedNetworking')
        ? networkInterfaceInformation[i].enableAcceleratedNetworking
        : false
      loadBalancerBackendAddressPoolResourceIds: contains(
          networkInterfaceInformation[i],
          'loadBalancerBackendAddressPoolResourceIds'
        )
        ? networkInterfaceInformation[i].loadBalancerBackendAddressPoolResourceIds
        : []
      loadBalancerInboundNatRuleResourceIds: contains(
          networkInterfaceInformation[i],
          'loadBalancerInboundNatRuleResourceIds'
        )
        ? networkInterfaceInformation[i].loadBalancerInboundNatRuleResourceIds
        : []
      location: contains(networkInterfaceInformation[i], 'location')
        ? networkInterfaceInformation[i].location
        : 'westeurope'
      ipConfigurations: contains(networkInterfaceInformation[i], 'ipConfigurations')
        ? networkInterfaceInformation[i].ipConfigurations
        : []
      privateIPAllocationMethod: contains(networkInterfaceInformation[i], 'privateIPAllocationMethod')
        ? networkInterfaceInformation[i].privateIPAllocationMethod
        : ''
      subnetResourceId: contains(networkInterfaceInformation[i], 'subnetResourceId')
        ? networkInterfaceInformation[i].subnetResourceId
        : ''
