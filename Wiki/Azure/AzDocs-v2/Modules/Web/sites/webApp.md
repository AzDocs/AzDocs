...
| webAppKind | string | <input type="checkbox"> | `'api'` or  `'app'` or  `'app,linux'` or  `'app,linux,container'` or  `'hyperV'` or  `'app,container,windows'` or  `'app,linux,kubernetes'` or  `'functionapp'` or  `'functionapp,linux'` or  `'functionapp,linux,kubernetes'` or  `'functionapp,linux,kubernetes,container'` | <pre>'app,linux'</pre> | The type of webapp to create. Defaults to a Linux App Service Instance. |
