...
| webAppKind | string | <input type="checkbox"> | `'api'` or `'app'` or `'app,linux'` or `'app,linux,container'` or `'hyperV'` or `'app,container,windows'` or `'app,linux,kubernetes'` or `'functionapp'` or `'functionapp,powershell'` or `'functionapp,linux'` or `'functionapp,linux,kubernetes'` or `'functionapp,linux,kubernetes,container'` or `'functionapp,linux,powershell'` | <pre>'app,linux'</pre> | The type of webapp to create. |
