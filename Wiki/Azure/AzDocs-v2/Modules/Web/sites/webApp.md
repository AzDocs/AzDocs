...
| ipSecurityRestrictions | array | <input type="checkbox"> | None | <pre>[<br>  {<br>    ipAddress: '0.0.0.0/0'<br>    action: 'Deny'<br>    tag: 'Default'<br>    priority: 10<br>    name: 'DefaultDeny'<br>    description: 'Default deny to make sure that something isnt publicly exposed on accident.'<br>  }<br>]</pre> | IP security restrictions for the main entrypoint. Defaults to closing down the appservice instance for all connections. For object format, please refer to https://docs.microsoft.com/en-us/azure/templates/microsoft.web/sites?tabs=bicep#ipsecurityrestriction. |
