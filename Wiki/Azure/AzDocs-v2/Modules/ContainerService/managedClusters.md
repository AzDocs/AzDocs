...
| nodePoolVmOsSKU | string | <input type="checkbox"> | None | <pre>nodePoolVmOsType == 'Linux' ? 'AzureLinux' : 'Windows'</pre> | Specifies the OS SKU used by the agent pool. If not specified, the default is Ubuntu if OSType=Linux or Windows2019 if OSType=Windows.<br>And the default Windows OSSKU will be changed to Windows2022 after Windows2019 is deprecated. |
...
| omsagent | bool | <input type="checkbox"> | None | <pre>!empty(logAnalyticsWorkspaceResourceId)</pre> | Enable AKS logs |
...
| enableSysLog | bool | <input type="checkbox"> | None | <pre>!empty(logAnalyticsWorkspaceResourceId)</pre> | Enable SysLogs and send to log analytics |
| networkProfileNetworkPlugin | string | <input type="checkbox"> | `'kubenet'` or `'azure'` or `'none'` | <pre>'azure'</pre> | Network plugin used for building the Kubernetes network. |
| networkPluginMode | string | <input type="checkbox"> | `''` or `'overlay'` | <pre>'overlay'</pre> | The network plugin type |
...
| networkProfileOutboundType | string | <input type="checkbox"> | `'loadBalancer'` or `'managedNATGateway'` or `'userAssignedNATGateway'` or `'userDefinedRouting'` | <pre>'userDefinedRouting'</pre> | Egress outbound type. Can only be set at creation time. |
...
| networkProfilePodCidrs | array | <input type="checkbox"> | None | <pre>!empty(ipv6PodCidr)</pre> | One IPv4 CIDR is expected for single-stack networking. Two CIDRs, one for each IP family (IPv4/IPv6), is expected for dual-stack networking.<br>Example:<br>[<br>&nbsp;&nbsp;&nbsp;'100.64.0.0/16'<br>&nbsp;&nbsp;&nbsp;'fdfd:fdfd:0:2::/64'<br>] |
| ipv6ServiceCidr | string | <input type="checkbox"> | None | <pre>''</pre> | The ipv6 servicecidr for a dual-stack aks cluster.<br>Example:<br>'fdfd:fdfd:0:3::/108' |
| serviceCidrs | array | <input type="checkbox"> | None | <pre>!empty(ipv6ServiceCidr)</pre> | One IPv4 CIDR is expected for single-stack networking. Two CIDRs, one for each IP family (IPv4/IPv6), is expected for dual-stack networking. They must not overlap with any Subnet IP ranges |
| networkProfileIpFamilies | array | <input type="checkbox"> | `'IPv4'` or `'IPv6'` | <pre>[<br>  'IPv4'<br>]</pre> | IP families are used to determine single-stack or dual-stack clusters. For single-stack, the expected value is IPv4.<br>For dual-stack, the expected values are IPv4 and IPv6. If you configure this with both IPv4 and IPv6 and you do not set a byo network, the network will be created with ipv4 and ipv6.<br>And serviceCidrs and podsCidrs will also be configured automatically with both.<br>Otherwise with byo network, this is expected to be present. |
...
| dnsPrefix | string | <input type="checkbox"> | None | <pre>aksName</pre> |  |
| enablePrivateCluster | bool | <input type="checkbox"> | None | <pre>true</pre> | Whether you want your aks cluster to be private. When true, must be used with fqdnSubdomain. |
| apiServerSubnetId | string | <input type="checkbox"> | None | <pre>''</pre> | The subnet to integrate the API server into. If empty, the API server will not be integrated into a subnet. |
...
| managedOutboundIPsIPv6 | int | <input type="checkbox"> | Value between 0-100 | <pre>0</pre> | The desired number of IPv6 outbound IPs created/managed by Azure for the cluster load balancer. Allowed values must be in the range of 0 to 100 (inclusive). The default value is 0 for single-stack and 1 for dual-stack. |
